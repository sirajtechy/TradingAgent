"""
core.universe — Stock universes, backtest time windows, and confusion-matrix helpers.

Canonical location for:
  - MONTHS          : Standard 12-month backtest window
  - SECTORS         : Legacy 5-sector, 10-ticker universe (non-halal)
  - HALAL_SECTORS   : 12-sector halal universe from Musaffa
  - Confusion matrix functions for backtest evaluation

Migrated from backtests/common.py (Phase 5 engine shrink).
"""
...
try:
    ...
    HALAL_SECTORS: Dict[str, List[str]] = {}  # type: ignore[no-redef]
    HALAL_ALL_TICKERS: List[str] = []
    def load_sector_tickers(sector: str) -> List[str]:  # type: ignore[misc]
        return []
    def load_all_tickers() -> List[str]:  # type: ignore[misc]
        return []
...
def empty_matrix() -> Dict[str, int]:
    """Return a fresh confusion matrix with zeroed counts."""
