...
then for each (signal_date, result_date) pair:
  ...
  4. Fetch the month-end closing price as of *result_date* via yfinance.
  5. Compare predicted vs actual direction → signal_correct True/False/None.

This design makes the backtest  ~12× faster than calling analyze_ticker()
per period (1 API round-trip per ticker vs 12).

Extra O'Neil-specific metrics tracked per period
──────────────────────────────────────────────────
  - pattern_detected   (Cup with Handle | Double Bottom | … | None)
  - pattern_confidence (0.0–1.0)
  - is_late_stage      (bool)
  - volume_dry_up      (bool)
  - market_stage       (1–4)
  - confluence_score   (0–4)
  - entry_price        (pivot level)
  - stop_loss          (−7% of entry)
  - risk_reward_ratio

These enable slice-and-dice analysis by pattern type, stage, and confidence.

Data source priority
────────────────────
  1. Polygon.io OHLCV bars (US tickers only)
  2. yfinance fallback (all tickers, including NSE)

End prices always fetched via yfinance (month-end spot price).
"""
...
def _fetch_end_price(ticker: str, result_date: date) -> Optional[Tuple[float, str]]:
    ...
    try:
        ...
        if df is None or df.empty:
            return None
        # Take last row — closest to result_date from below
