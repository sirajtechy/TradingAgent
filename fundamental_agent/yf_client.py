...
# yfinance prints noisy warnings; suppress them for cleaner output
...
class YFinanceClient:
    ...
    def _build_price_point(self, ticker_obj: yf.Ticker, ticker: str, as_of_date: date) -> PricePoint:
        ...
        try:
            ...
            # yfinance 1.2.x raises on None chart result for some tickers
            hist = None

        if hist is None or (hasattr(hist, "empty") and hist.empty):
            # Fallback: use yf.download which has a different code path
            try:
