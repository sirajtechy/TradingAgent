#!/usr/bin/env python3
"""
Sector-wide 12-month technical backtest.

Covers March 2025 – February 2026 (12 complete months).
Five sectors, 12 hand-picked large/mid-cap stocks each = 60 tickers.

Outputs:
  - Per-ticker results JSON  (in ``--output-dir``)
  - technical_sector_backtest_results.json  (all raw data)
  - technical_sector_confusion_matrix.json  (metrics per sector + overall)
  - Console report with confusion matrices formatted as tables

Usage:
    python run_technical_sector_backtest.py
    python run_technical_sector_backtest.py --sector Technology
    python run_technical_sector_backtest.py --resume
"""
...
# ---------------------------------------------------------------------------
# Sector universe  —  same 60 tickers as the fundamental backtest
# ---------------------------------------------------------------------------
...
def _update_matrix(matrix: Dict[str, int], period: Dict[str, Any]) -> None:
    """Classify a single period into the confusion matrix."""
...
def _matrix_metrics(m: Dict[str, int]) -> Dict[str, Any]:
    ...
    total = directional + m["neutral"]
    abstention_rate = m["neutral"] / total if total > 0 else None
    ...
    return {
        ...
        "specificity_pct": (
            round(specificity * 100, 1) if specificity is not None else None
        ),
        "f1_pct": round(f1 * 100, 1) if f1 is not None else None,
        "abstention_rate_pct": (
            round(abstention_rate * 100, 1)
...
def _build_matrix_from_results(
    sector_results: Dict[str, Any],
) -> Dict[str, int]:
    """Re-derive a confusion matrix from stored period data."""
    m = _empty_matrix()
...
def run_ticker(
    ...
    """
    Run (or resume) a 12-month technical backtest for *ticker*.

    Args:
        ticker:     Stock symbol.
        resume:     If True, skip if cached JSON exists.
        output_dir: Directory for per-ticker JSON files.

    Returns:
        Backtest result dict, or None on failure.
    """
    out_file = output_dir / f"{ticker}_technical_backtest_results.json"
    ...
    try:
        ...
        hit_str = (
            f"{acc}% ({result['summary']['correct_signals']}/{sigs})"
            if acc is not None
            else "N/A (all neutral)"
        )
...
def _render_matrix_table(metrics: Dict[str, Any], label: str) -> str:
    """Render a confusion matrix + metrics as a formatted text block."""
    lines: List[str] = []
    lines.append(f"\n{'─' * 60}")
    ...
    lines.append(
        f"  Neutral abstentions : {metrics['neutral_count']}  "
        f"({_fmt(metrics['abstention_rate_pct'])} of all decisions)"
    )
    lines.append(f"  Accuracy            : {_fmt(metrics['accuracy_pct'])}")
    lines.append(
        f"  Precision  (TP/(TP+FP)): {_fmt(metrics['precision_pct'])}"
    )
    lines.append(
        f"  Recall     (TP/(TP+FN)): {_fmt(metrics['recall_pct'])}"
    )
    lines.append(
        f"  Specificity(TN/(TN+FP)): {_fmt(metrics['specificity_pct'])}"
    )
    lines.append(f"  F1 Score            : {_fmt(metrics['f1_pct'])}")
    if metrics["error_count"] > 0:
        lines.append(
            f"  ⚠  Tickers with errors (excluded): {metrics['error_count']}"
        )
...
def _render_ticker_summary_table(sector_results: Dict[str, Any]) -> str:
    """Print a per-ticker summary table for a single sector."""
    lines: List[str] = []
    lines.append(
    ...
    for ticker, data in sorted(sector_results.items()):
        ...
        s = data["summary"]
        acc = (
            f"{s['accuracy_pct']}%" if s["accuracy_pct"] is not None else "N/A"
        )
        sigs = s["directional_signals"]
        corr = s["correct_signals"]
        neut = s["total_periods"] - sigs
        scores = [
            p["experimental_score"]
            for p in data["periods"]
            if p.get("experimental_score") is not None
        ]
        avg_score = f"{sum(scores) / len(scores):.1f}" if scores else "N/A"
        bands = [
            p["score_band"] for p in data["periods"] if p.get("score_band")
        ]
        common_band = max(set(bands), key=bands.count) if bands else "N/A"
        lines.append(
            f"  {ticker:<8}  {sigs:>7}  {corr:>7}  {acc:>6}  {neut:>7}  "
...
def main() -> None:
    """Entry-point for the sector-wide 12-month technical backtest."""
    parser = argparse.ArgumentParser(
        description="Sector-wide 12-month technical backtest with "
                    "confusion matrices."
    )
    parser.add_argument(
        "--sector", help="Run only this sector (default: all five)"
    )
    ...
    print(f"\n{'=' * 64}")
    print("  TECHNICAL SECTOR BACKTEST — 12 months (Mar 2025 – Feb 2026)")
    ...
    for sector_name, tickers in sectors_to_run.items():
        ...
        for i, ticker in enumerate(tickers, 1):
            ...
            result = run_ticker(ticker, args.resume, output_dir)
    ...
    with open("technical_sector_backtest_results.json", "w") as fh:
        json.dump(consolidated, fh, indent=2, default=str)
    print("\n✓  Full results saved to technical_sector_backtest_results.json")
    ...
    with open("technical_sector_confusion_matrix.json", "w") as fh:
        json.dump(cm_output, fh, indent=2)
    print("✓  Confusion matrices saved to technical_sector_confusion_matrix.json")
