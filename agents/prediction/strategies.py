...
# ── Strategy weights (loaded at import time) ──────────────────────────────────
# derive_strategy_weights.py writes agents/prediction/strategy_weights.json.
# Keys are horizon strings (e.g. "30", "60").  If the file is absent or
# unreadable, ml_layer() falls back to the hardcoded defaults below.

_WEIGHTS_FILE = Path(__file__).resolve().parent / "strategy_weights.json"
_CACHED_WEIGHTS: Optional[Dict[str, Dict[str, float]]] = None


def _load_weights_file() -> Dict[str, Dict[str, float]]:
    """Load strategy_weights.json once and cache. Returns empty dict on any error."""
    global _CACHED_WEIGHTS
    if _CACHED_WEIGHTS is not None:
        return _CACHED_WEIGHTS
    try:
        import json
        _CACHED_WEIGHTS = json.loads(_WEIGHTS_FILE.read_text())
    except Exception:
        _CACHED_WEIGHTS = {}
    return _CACHED_WEIGHTS


def _get_strategy_weights(horizon_days: int = 30) -> Dict[str, float]:
    """
    Return strategy weights for *horizon_days*, loading from the JSON file when
    available.  If the key is missing or the file is absent, fall back to
    hardcoded defaults.

    The fallback values below were derived from initial backtests and should be
    replaced by running ``scripts/derive_strategy_weights.py`` on fresh data.
    """
    weights_data = _load_weights_file()

    # Try exact horizon key, then "all" (cross-horizon aggregate), then fallback
    for key in (str(horizon_days), "all"):
        weights = weights_data.get(key)
        if weights and isinstance(weights, dict):
            return weights

    # ── Hardcoded fallback (ordered by measured accuracy from initial backtest) ──
    return {
        "RSI Divergence":                 0.18,   # 63.2% acc — highest
        "Ichimoku Cloud":                 0.15,   # 56.1% acc
        "Supertrend":                     0.14,   # 56.3% acc
        "EMA Crossover":                  0.12,   # 51.3% acc
        "MACD + RSI":                     0.10,   # 50.0% acc
        "S/R Breakout":                   0.09,   # 50.0% acc
        "Mean Reversion":                 0.08,   # 50.0% acc
        "Bollinger Squeeze":              0.07,   # 100% acc but n=1 — low trust
        "OBV Divergence":                 0.07,   # 40.0% acc — demoted
    }
...
def ml_layer(strategy_signals: List[Dict[str, Any]], df: pd.DataFrame) -> Dict[str, Any]:
    ...
    Weights are loaded from ``agents/prediction/strategy_weights.json`` when
    available (generated by ``scripts/derive_strategy_weights.py``).  If the
    file is absent, evidence-based fallback weights are used instead.

    In a production system this would be a trained XGBoost model on
    historical backtested signal-outcome pairs.
    """
    try:
        STRATEGY_WEIGHTS = _get_strategy_weights(horizon_days=30)
